...
import {
    PencilRuler, Rocket, ShieldCheck, Cpu, Layers, Smartphone, ChevronRight, Sparkles
} from "lucide-react";
...
const BentoCard = ({
    ...
    const initial =
        direction === "left" ? { opacity: 0, x: -40, scale: 0.97 }
        : direction === "right" ? { opacity: 0, x: 40, scale: 0.97 }
    ...
    return (
        <motion.div
            ...
            className={`relative group bg-white/[0.02] border border-white/5 rounded-3xl overflow-hidden p-8 hover:bg-white/[0.04] hover:border-accent-mint/20 transition-all duration-500 ${className}`}
        >
        ...
        <section id="services" className="w-full py-10 px-6 md:px-12 max-w-7xl mx-auto overflow-hidden scroll-mt-20">
            ...
            <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <motion.p
                    ...
                    className="text-text-muted max-w-md text-sm md:text-base md:text-right"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-auto md:h-[650px]">
                {/* Enterprise Web */}
                <BentoCard title="Enterprise Web" subtitle="Scalable Architecture" icon={Rocket} className="md:col-span-2 md:row-span-1" delay={0.08} direction="left">
                ...
                {/* Design Systems */}
                <BentoCard title="Design Systems" subtitle="Consistency at Scale" icon={PencilRuler} className="md:col-span-1 md:row-span-2" delay={0.14} direction="right">
                        ...
                        <div className="mt-auto pt-6 flex justify-between items-center border-t border-white/5">
                            <div className="flex gap-3">
                                {['Figma', 'Storybook', 'Tailwind'].map((tool) => (
                                    <span key={tool} className="text-[9px] font-mono text-text-muted hover:text-accent-mint transition-colors cursor-default uppercase">{tool}</span>
                                ))}
                            </div>
                ...
                {/* Mobile */}
                <BentoCard title="Mobile Dev" subtitle="iOS & Android Solutions" icon={Smartphone} className="md:col-span-1 md:row-span-1" delay={0.2} direction="left">
                    <div className="space-y-4 h-full flex flex-col">
                        <div className="flex flex-wrap gap-2">
                            {['React Native', 'Flutter', 'Fastlane'].map((tech) => (
                                <span key={tech} className="text-[9px] px-2 py-0.5 rounded bg-white/5 border border-white/10 text-text-muted">{tech}</span>
                            ))}
                        </div>
                        ...
                        <div className="group/btn flex items-center justify-between bg-accent-mint/5 border border-accent-mint/10 p-4 rounded-2xl hover:bg-accent-mint/10 hover:border-accent-mint/30 transition-all cursor-pointer mt-auto">
                            ...
                            <ChevronRight className="w-4 h-4 text-accent-mint group-hover/btn:translate-x-1 transition-transform" />
                        </div>
                    </div>
                </BentoCard>

                {/* AI */}
                <BentoCard title="AI Integration" subtitle="Applied Intelligence" icon={Cpu} className="md:col-span-1 md:row-span-1" delay={0.26} direction="up">
                    <div className="space-y-4 h-full flex flex-col mt-2">
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2 text-[11px] text-text-secondary">
                                <Layers className="w-3.5 h-3.5 text-accent-mint" /> RAG Architecture
                            </div>
                            <div className="flex items-center gap-2 text-[11px] text-text-secondary">
                                <ShieldCheck className="w-3.5 h-3.5 text-accent-mint" /> Prompt Engineering
                            </div>
                        </div>
