...
import {
    Code2, Github, Layout, Zap, Globe, Cpu, Terminal, CheckCircle2
} from "lucide-react";
...
const BentoItem = ({
    ...
    const initial =
        direction === "left" ? { opacity: 0, x: -30, scale: 0.97 }
        : direction === "right" ? { opacity: 0, x: 30, scale: 0.97 }
        : direction === "scale" ? { opacity: 0, scale: 0.92 }
    ...
    return (
        <motion.div
            ...
            whileHover={{ y: direction === "scale" ? 0 : -3, transition: { duration: 0.2, ease: "easeOut" } }}
        ...
        <section id="ecosystem" className="w-full py-10 px-6 md:px-12 max-w-7xl mx-auto overflow-hidden relative scroll-mt-20">
            ...
            <div className="mb-16">
                ...
                <motion.p
                    ...
                    className="text-text-muted mt-4 max-w-xl text-lg"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-3 gap-4 h-auto md:h-[750px]">
                {/* Main Card */}
                <BentoItem title="Fullstack Engineering" subtitle="End-to-end Architecture" icon={Code2} className="md:col-span-2 md:row-span-2" delay={0.08} direction="left">
                    <div className="mt-4 flex flex-col h-full">
                        <div className="grid grid-cols-2 gap-3 mb-4">
                            <div className="p-3 bg-white/5 rounded-xl border border-white/5 hover:border-accent-mint/20 transition-colors">
                ...
                {/* GitHub */}
                <BentoItem title="Open Source" subtitle="Active contributions" icon={Github} className="md:col-span-1 md:row-span-1" delay={0.16} direction="up">
                    <div className="flex flex-col h-full justify-center pt-1">
                        <div className="grid grid-cols-7 gap-1">
                            {Array.from({ length: 21 }).map((_, i) => (
                                <div key={i} className={`w-full aspect-square rounded-[2px] transition-colors duration-500 ${i % 3 === 0 ? 'bg-accent-mint/60' : i % 4 === 0 ? 'bg-accent-mint/20' : 'bg-white/5'}`} />
                            ))}
                        </div>
                ...
                {/* User Centric */}
                <BentoItem title="User Centric" subtitle="Design Philosophy" icon={Layout} className="md:col-span-1 md:row-span-2" delay={0.2} direction="right">
                ...
                {/* Performance */}
                <BentoItem title="Optimization" subtitle="Core Web Vitals" icon={Zap} className="md:col-span-1 md:row-span-1" delay={0.24} direction="up">
                    <div className="flex flex-col items-center justify-center h-full">
                        ...
                        <div className="flex gap-3 mt-4 w-full justify-between items-center px-1">
                            {[['LCP', '0.8s'], ['SEO', '100'], ['FID', '12ms']].map(([k, v]) => (
                                <div key={k} className="flex flex-col items-center">
                                    <span className="text-[8px] text-text-muted">{k}</span>
                                    <span className="text-[9px] text-accent-mint font-bold">{v}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {/* Global */}
                <BentoItem title="Availability" subtitle="Collaboration" icon={Globe} className="md:col-span-2 md:row-span-1" delay={0.28} direction="left">
                    <div className="flex flex-col md:flex-row md:items-end justify-between h-full pt-1 gap-4">
                        ...
                        <div className="flex -space-x-2 pb-1">
                            {['AZ', 'BK', 'SM', 'RJ'].map((initials, i) => (
                                <div key={i} className="w-8 h-8 rounded-full border-2 border-theme-dark bg-white/10 flex items-center justify-center text-[10px] font-bold text-white shadow-lg">{initials}</div>
                            ))}
                            <div className="w-8 h-8 rounded-full border-2 border-theme-dark bg-accent-mint/20 text-accent-mint flex items-center justify-center text-[10px] font-bold shadow-lg">+12</div>
                        </div>
                    </div>
                </BentoItem>

                {/* Terminal */}
                <BentoItem className="md:col-span-1 md:row-span-1 border-white/10 bg-black/60 shadow-2xl" delay={0.32} direction="scale">
                ...
                {/* Mindset */}
                <BentoItem title="Mindset" subtitle="Problem Solving" icon={Cpu} className="md:col-span-1 md:row-span-1" delay={0.36} direction="right">
